...
data_size_str=$1
workload_file="load/${data_size_str}_load.bin"
...
if [ $PROGRAM_EXIT_CODE -ne 0 ]; then
    echo "${data_size_str} load workload failed with exit code ${PROGRAM_EXIT_CODE}"
else
    echo "${data_size_str} load workload successfully"
fi