...
# ../../../generator/generator --ranges 87380 > 1mb_range.txt
...
echo "[BENCHMARK] ranges script in directory: $script_dir"

# 1, 1MB, 10MB, 25MB, 100MB, 500MB
data_size_str=$1
workload_file="range/${data_size_str}_range.txt"
...
if [ $PROGRAM_EXIT_CODE -ne 0 ]; then
    echo "${data_size_str} range workload failed with exit code ${PROGRAM_EXIT_CODE}"
else
    echo "${data_size_str} range workload successfully"
fi
